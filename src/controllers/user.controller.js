...
if([fullName, password, email ,username].some((field)=> field?.trim()==="")) 
  ...
  //selct ka use jo fields nahi cahiye DB se vo dete h - ke sath
...
// Hear is an alternative of avobe codebased on logic discussed in video:
// if(!username && ! email){
//throw new ApiError(400, "username or email is required")
    //  }
...
const updateUserAvatar = asyncHandler(async(req,res)=>{
  ...
  .status(200)
  .json(
    new ApiResponse(200, user, "avatar Image Uplosded Successfully")
  )
})
...
const updateUserCoverImage = asyncHandler(async(req,res)=>{
  ...
  .status(200)
  .json(
    new ApiResponse(200, user, "Cover Image Uplosded Successfully")
  )
})
...
.json(
  new ApiResponse(
    ...
    "watched history fetched succeffully"
  )
)
})
